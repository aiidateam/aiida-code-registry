...
# See https://stackoverflow.com/a/53394430/1069467
def freezeargs(func):
    ...
    try:
        ...
        if label not in computer_list:
            raise common.NotExistent(
                f"Computer '{label}' found neither in database nor in registry. "
                + f"Computers in registry: {pprint.pformat(computer_list)}"
            ) from exc
...
def load_code(label: str, template_vars: dict = None) -> orm.Code:
    ...
    try:
        ...
        if label not in code_list:
            raise common.NotExistent(
                f"Code '{label}' found neither in database nor in registry. Codes in registry:\n{pprint.pformat(code_list)}"
            ) from exc
